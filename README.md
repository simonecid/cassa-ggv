...
Sistema di cassa per il festival GGV. Ogni postazione (cassa) gira interamente nel browser; il server centrale ospita CouchDB e serve i file statici.

## Architettura

```
SERVER CENTRALE (qualsiasi macchina)
├── CouchDB          porta 5984
...
Il browser usa la **WebUSB API** per comunicare direttamente con la stampante ESC/POS via USB.
...
cassa-ggv/
├── couchdb/
│   └── local.ini            configurazione CouchDB (copiare in /etc/couchdb/local.ini)
├── serverpython/
...
└── www/
    ...
    └── db/
        ├── menu.js          catalogo prodotti (modifica qui per aggiornare il menu)
        └── opzioni.js       valori predefiniti delle opzioni
```
